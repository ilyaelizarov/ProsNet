within ProsNet.Fluid.Pumps.BaseClasses;
partial model PartialFlowMachine
  "Partial model to interface fan or pump models with the medium"
  extends ProsNet.Fluid.Interfaces.LumpedVolumeDeclarations(
    final mSenFac=1);
  extends ProsNet.Fluid.Interfaces.PartialTwoPortInterface(
  ...
  replaceable parameter ProsNet.Fluid.Pumps.Data.Generic per
    constrainedby ProsNet.Fluid.Pumps.Data.Generic
    "Record with performance data"
    annotation (choicesAllMatching=true,
      Placement(transformation(extent={{52,60},{72,80}})));

  parameter ProsNet.Fluid.Types.InputType inputType = ProsNet.Fluid.Types.InputType.Continuous
    "Control input type"
    annotation(Dialog(
      group="Control"));
  parameter Real constInput = 0 "Constant input set point"
    annotation(Dialog(
      group="Control",
      enable=inputType == ProsNet.Fluid.Types.InputType.Constant));
  parameter Real stageInputs[:]
    "Vector of input set points corresponding to stages"
    annotation(Dialog(
      group="Control",
      enable=inputType == ProsNet.Fluid.Types.InputType.Stages));
  ...
  parameter Boolean nominalValuesDefineDefaultPressureCurve = true
  ...
  // Connectors and ports
    ...
    if inputType == ProsNet.Fluid.Types.InputType.Stages
...
protected
  ...
  // preVar == ProsNet.Fluid.Pumps.BaseClasses.Types.PrescribedVariable.Speed,
  ...
  // ProsNet.Fluid.Movers.SpeedControlled_y$floMac1.
  ...
  Modelica.Blocks.Sources.Constant[size(stageInputs, 1)] stageValues(
    final k=stageInputs)
   if inputType == ProsNet.Fluid.Types.InputType.Stages "Stage input values"
    annotation (Placement(transformation(extent={{-80,40},{-60,60}})));
  Modelica.Blocks.Sources.Constant setConst(
    final k=constInput)
   if inputType == ProsNet.Fluid.Types.InputType.Constant
    "Constant input set point"
    annotation (Placement(transformation(extent={{-80,70},{-60,90}})));

  Extractor extractor(final nin=size(stageInputs,1))
   if inputType == ProsNet.Fluid.Types.InputType.Stages "Stage input extractor"
  ...
  ProsNet.Fluid.Delays.DelayFirstOrder vol(
 ...
 if inputType == ProsNet.Fluid.Types.InputType.Continuous and
    speedIsInput
    "Gain to normalized speed using speed_nominal or speed_rpm_nominal"
    annotation (Placement(transformation(extent={{-4,74},{-16,86}})));

  ProsNet.Fluid.Pumps.BaseClasses.IdealSource preSou(
    redeclare final package Medium = Medium,
    final m_flow_small=m_flow_small,
    final allowFlowReversal=allowFlowReversal,
    final control_m_flow=(preVar == ProsNet.Fluid.Pumps.BaseClasses.Types.PrescribedVariable.FlowRate))
    "Pressure source"
    annotation (Placement(transformation(extent={{40,-10},{60,10}})));

  ProsNet.Fluid.Pumps.BaseClasses.PowerInterface heaDis(final
 ...
 if addPowerToMedium
  ...
  ProsNet.Fluid.Sensors.MassFlowRate senMasFlo(
    redeclare final package Medium = Medium) "Mass flow rate sensor"
    annotation (Placement(transformation(extent={{-50,10},{-30,-10}})));

  ProsNet.Fluid.Sensors.RelativePressure senRelPre(redeclare final package
      Medium =
...
initial equation
  ...
  assert(
    nominalValuesDefineDefaultPressureCurve or per.havePressureCurve or (preVar ==
      ProsNet.Fluid.Pumps.BaseClasses.Types.PrescribedVariable.Speed),
...
equation
  connect(prePow.port, vol.heatPort) annotation (Line(
      points={{-34,-94},{-60,-94},{-60,10},{-69.6,10}},
      color={191,0,0}));

  connect(vol.heatPort, heatPort) annotation (Line(
      points={{-69.6,10},{-69.6,10},{-60,10},{-60,-100}},
      color={191,0,0}));
