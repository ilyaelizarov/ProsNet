...
model SF1_NewDemand "Example model of a building with loads provided as time series and
  ...
  Modelica.Blocks.Interfaces.RealOutput TTop
    ...
    annotation (Placement(transformation(extent={{340,202},{378,240}}),
        iconTransformation(extent={{340,202},{378,240}})));
  Modelica.Blocks.Interfaces.RealOutput TMiddle
    "Absolute temperature as output signal"
    annotation (Placement(transformation(extent={{340,164},{376,200}}),
        iconTransformation(extent={{340,164},{376,200}})));
  Modelica.Blocks.Interfaces.RealOutput TBottom
    "Absolute temperature as output signal"
    annotation (Placement(transformation(extent={{338,134},{372,168}}),
        iconTransformation(extent={{338,134},{372,168}})));
  Modelica.Fluid.Interfaces.FluidPort_a port_a2
    "Fluid connector a (positive design flow direction is from port_a to port_b)"
    annotation (Placement(transformation(extent={{-236,266},{-126,338}}),
        iconTransformation(extent={{-330,194},{-144,352}})));
  Modelica.Fluid.Interfaces.FluidPort_b port_b2
    "Fluid connector b (positive design flow direction is from port_a to port_b)"
    annotation (Placement(transformation(extent={{-42,278},{78,360}}),
  ...
  Modelica.Fluid.Sensors.TemperatureTwoPort T_ret_gen2(redeclare package Medium
      = Modelica.Media.Water.ConstantPropertyLiquidWater, allowFlowReversal=
        ...
        rotation=180,
        origin={-380,-201})));
  Components.Consumers.Demand_Model_Radiator demand_Model_Radiator
    annotation (Placement(transformation(extent={{252,-90},{322,-26}})));
equation
  ...
  connect(souCol.ports[1], theMixVal.port_col) annotation (Line(points={{190,
          -186},{200,-186},{200,-238.4},{254,-238.4}},
                                                 color={0,127,255}));
  ...
  connect(ChpControl.CHPON, wolfCGB14_GC1.CBOn1) annotation (Line(points={{-632.62,
        ...
        color={255,0,255}));
  connect(tan.T1, TTop) annotation (Line(points={{18.72,27.68},{72,27.68},{72,
          221},{359,221}},        color={0,0,127}));
  connect(tan.T5, TMiddle) annotation (Line(points={{18.13,-4.77},{24,-4.77},{
          24,182},{358,182}},              color={0,0,127}));
  connect(tan.T10, TBottom) annotation (Line(points={{18.13,-47.25},{216,-47.25},
          {216,150},{348,150},{348,151},{355,151}},
                                            color={0,0,127}));
  connect(TTop, TTop)
    annotation (Line(points={{359,221},{359,221}}, color={0,0,127}));
  connect(port_b2,port_b2)
    annotation (Line(points={{18,319},{18,319}}, color={0,127,255}));
  ...
  connect(heatFlowRate_consume.Tcold, T_ret_gen1.T) annotation (Line(points={{425.72,
          54.7},{416,54.7},{416,-16},{362,-16},{362,-8.9}},
        color={0,0,127}));
  ...
  connect(T_sens_sec_hot2.T, heatFlowRate_Chp.Tcold) annotation (Line(points={{
          ...
          -309.3}}, color={0,0,127}));
  connect(T_ret_gen1.port_a, demand_Model_Radiator.port_b1) annotation (Line(
        points={{376,1},{384,1},{384,-75.92},{357,-75.92}}, color={0,127,255}));
  connect(T_sens_sec_hot1.port_b, demand_Model_Radiator.port_a1) annotation (
      Line(points={{212,41},{238,41},{238,-75.92}}, color={0,127,255}));
  connect(demand_Model_Radiator.weaBus1, weaDat.weaBus) annotation (Line(
      points={{260.4,-33.68},{260.4,130},{-48,130}},
      color={255,204,51},
      thickness=0.5));
  connect(tan.fluPorVol1[10], domHotWatTan.port_aDom) annotation (Line(points={
          {5.74,8.8},{48,8.8},{48,-278.4},{76,-278.4}}, color={0,127,255}));
  connect(port_b2, tan.fluPorVol1[2]) annotation (Line(points={{18,319},{18,160},
          {32,160},{32,-10.08},{5.74,-10.08}}, color={0,127,255}));
  connect(port_a2, tan.fluPorVol1[2]) annotation (Line(points={{-181,302},{-181,
          168},{16,168},{16,160},{32,160},{32,-10.08},{5.74,-10.08}}, color={0,
          127,255}));
