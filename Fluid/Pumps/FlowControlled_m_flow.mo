...
*/
  ...
  extends ProsNet.Fluid.Pumps.BaseClasses.PartialFlowMachine(
    ...
    final preVar=ProsNet.Fluid.Pumps.BaseClasses.Types.PrescribedVariable.FlowRate,
    ...
    eff(
      per(
        final pressure = if per.havePressureCurve then
          ...
          ProsNet.Fluid.Pumps.BaseClasses.Characteristics.flowParameters(
  ...
  parameter Modelica.Units.SI.MassFlowRate constantMassFlowRate=m_flow_nominal
    "Constant pump mass flow rate, used when inputType=Constant" annotation (
      Dialog(enable=inputType == ProsNet.Fluid.Types.InputType.Constant));
  ...
  parameter Modelica.Units.SI.MassFlowRate[:] massFlowRates=m_flow_nominal*{per.speeds[
    ...
    annotation (Dialog(enable=inputType == ProsNet.Fluid.Types.InputType.Stages));

  Modelica.Blocks.Interfaces.RealInput m_flow_in(
    final unit="kg/s",
    nominal=m_flow_nominal)
    if inputType == ProsNet.Fluid.Types.InputType.Continuous
  ...
  annotation (
      Icon(graphics={
        Text(
          ...
          lineColor={0,0,127},
          visible=inputType == ProsNet.Fluid.Types.InputType.Continuous or inputType == ProsNet.Fluid.Types.InputType.Stages,
          textString=DynamicSelect("m_flow", if inputType == ProsNet.Fluid.Types.InputType.Continuous then String(m_flow_in, leftJustified=false, significantDigits=3) else String(stage)))}),
