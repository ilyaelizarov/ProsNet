...
model SF1_control_validation "Example model of a building with loads provided as time series and
  ...
  Modelica.Blocks.Sources.Constant conTSetHot(k(
    ...
    annotation (Placement(transformation(extent={{8,-8},{-8,8}},
        rotation=0,
        origin={48,-300})));
  ProsNet.Fluid.Pumps.FlowControlled_m_flow pump(m_flow_nominal=10,
      addPowerToMedium=true)
    annotation (Placement(transformation(extent={{-118,-298},{-74,-264}})));
  ProsNet.Components.Generators.Digital_Twins.WolfCGB14_GC wolfCGB14_GC1
    annotation (Placement(transformation(extent={{-524,2},{-432,56}})));
  ProsNet.Components.Generators.Digital_Twins.NeoTower2_GC neoTower2_GC1
    annotation (Placement(transformation(extent={{-528,-194},{-436,-130}})));
  ProsNet.Fluid.Sources.Boundary_pT souCol(redeclare package Medium =
        ProsNet.Media.Water, nPorts=2)
    annotation (Placement(transformation(extent={{170,-196},{190,-176}})));

  ProsNet.Components.Generators.Laboratory_Models.PVSimpleOriented pv(
    A=200e3/800/0.12,
    til=0.34906585039887,
    azi=-0.78539816339745,
    V_nominal=480)
    annotation (Placement(transformation(extent={{476,-370},{564,-278}})));
  ProsNet.Components.Generators.Laboratory_Models.ASHRAE93 solCol(
  ...
  ProsNet.Components.Consumers.ThermostaticMixingValve theMixVal(
      mMix_flow_nominal=1.2*datWatHea.mDom_flow_nominal)
    annotation (Placement(transformation(extent={{254,-242},{294,-206}})));
  ProsNet.Components.Consumers.StorageTankWithExternalHeatExchanger domHotWatTan(
  ...
  ProsNet.Components.Electrical.Inductive acLoad2(linearized=false, mode=
        ProsNet.Fluid.Building_Fluid.Utili.Electrical.Types.Load.VariableZ_P_input)
    annotation (Placement(transformation(
        extent={{-22,-17},{22,17}},
        rotation=90,
        origin={-53,-446})));
  ProsNet.Components.Electrical.Inductive acLoad1(linearized=false, mode=
        ProsNet.Fluid.Building_Fluid.Utili.Electrical.Types.Load.VariableZ_P_input)
    annotation (Placement(transformation(
        extent={{-24,-16},{24,16}},
        rotation=90,
        origin={168,-446})));
  ProsNet.Components.Electrical.ACDCConverter conv(conversionFactor=480/480,
      eta=0.9)
    annotation (Placement(transformation(extent={{-72,-644},{16,-572}})));
  ProsNet.Components.Electrical.BatteryControl con
    annotation (Placement(transformation(extent={{-230,-644},{-198,-612}})));
  ProsNet.Components.Electrical.Battery bat(EMax=500e3*4*3600, V_nominal=480)
    annotation (Placement(transformation(extent={{-470,-652},{-374,-568}})));
  ProsNet.Components.Electrical.Grid gri(
    f=60,
    V=480,
    phiSou=0)
    annotation (Placement(transformation(extent={{12,-530},{88,-458}})));
  ProsNet.Components.Consumers.GenericDomesticHotWaterWithHeatExchanger datWatHea(VTan=
        0.1892706, mDom_flow_nominal=6.52944E-06*1000)
    annotation (Placement(transformation(extent={{440,-228},{460,-208}})));
  ProsNet.Controls.BooleanToReal booToRea(realTrue=2.5, realFalse=0)
    annotation (Placement(transformation(extent={{-132,-244},{-112,-224}})));
  ProsNet.Components.Valves.ThreeWayLinear val3(
    redeclare package Medium = ProsNet.Fluid.Building_Fluid.Utili.Media.Water,
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
  ...
  ProsNet.Components.Valves.ThreeWayLinear val4(
    redeclare package Medium = ProsNet.Fluid.Building_Fluid.Utili.Media.Water,
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
    use_inputFilter=false,
    m_flow_nominal=2,
  ...
  ProsNet.Components.Valves.ThreeWayLinear val1(
    redeclare package Medium = ProsNet.Fluid.Building_Fluid.Utili.Media.Water,
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
    use_inputFilter=false,
    m_flow_nominal=2,
    dpValve_nominal=1,
    l={0.05,0.05}) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={156,-142})));
  ProsNet.Components.BoundryCondition.ReaderTMY3 weaDat(filNam=
  ...
  ProsNet.Components.Storage.Stratified tan(
    redeclare package Medium = ProsNet.Fluid.Building_Fluid.Utili.Media.Water,
    m_flow_nominal=2,
    show_T=true,
    VTan(displayUnit="l") = 0.85,
    hTan=2,
    dIns=0.03,
    kIns=2,
    nSeg=10,
    ...
    TFlu_start={353.15,348.15,345.15,340.15,336.15,330.15,326.15,321.15,317.15,
        314.15})
    annotation (Placement(transformation(extent={{-104,-60},{14,58}})));
  Modelica.Blocks.Sources.BooleanExpression booleanExpression(y=true)
    annotation (Placement(transformation(extent={{-758,-58},{-736,-36}})));
  ProsNet.Controls.GeneratorControlPackage.ControlGenerator.GeneratorControl NeoTowerControl(TStartCHP
      =338, TStopCHP=353)
    annotation (Placement(transformation(extent={{-682,-214},{-614,-146}})));
  ProsNet.Controls.GeneratorControlPackage.ControlGenerator.GeneratorControl ChpControl(TStartCHP
      =333, TStopCHP=336)
    annotation (Placement(transformation(extent={{-694,6},{-628,72}})));
  Modelica.Fluid.Interfaces.FluidPort_a port_a1
    "Fluid connector a (positive design flow direction is from port_a to port_b)"
    annotation (Placement(transformation(extent={{-326,64},{-140,222}}),
        iconTransformation(extent={{-326,64},{-140,222}})));
  Modelica.Fluid.Interfaces.FluidPort_b port_b1
    "Fluid connector b (positive design flow direction is from port_a to port_b)"
    annotation (Placement(transformation(extent={{54,56},{252,224}}),
        iconTransformation(extent={{54,56},{252,224}})));
  Modelica.Blocks.Sources.Ramp y2(
    height=1,
    duration=1,
    offset=0) "Control signal"
    annotation (Placement(transformation(extent={{-110,174},{-90,194}})));
  ProsNet.Fluid.Valves.TwoWayEqualPercentage val5(
    redeclare package Medium = ProsNet.Media.Water,
    m_flow_nominal=10,
    dpValve_nominal=6000)
    annotation (Placement(transformation(extent={{-92,230},{-72,250}})));
  ProsNet.Fluid.Valves.TwoWayEqualPercentage val6(
    redeclare package Medium = ProsNet.Media.Water,
    m_flow_nominal=10,
    dpValve_nominal=6000)
    annotation (Placement(transformation(extent={{182,208},{202,228}})));
  ProsNet.Components.Consumers.BuildingRCZ1Valve buildingRCZ1Valve(
    facMul=26,
    nZon=3,
    nPorts_aHeaWat=1,
    nPorts_aChiWat=1,
...
equation
  ...
  connect(val1.port_2, tan.fluPorVol1[10]) annotation (Line(points={{146,-142},
          {88,-142},{88,-16},{2,-16},{2,10},{5.74,10},{5.74,10.8}},
                                                              color={0,127,255}));
  ...
  connect(ChpControl.TopTlayer, tan.T3) annotation (Line(points={{-703.24,49.56},
          {-720,49.56},{-720,96},{-608,96},{-608,-16},{-592,-16},{-592,-104},{
          -520,-104},{-520,-96},{17.54,-96},{17.54,13.16}},
                                                       color={0,0,127}));
  ...
  connect(val3.port_2, tan.fluPorVol[1]) annotation (Line(points={{-210,-20},{
          -210,-24},{-120,-24},{-120,-11.62},{-74.5,-11.62}}, color={0,127,255}));
  connect(buildingRCZ1Valve.ports_aHeaWat[1], tan.fluPorVol1[1]) annotation (
      Line(points={{242,-67.6},{176,-67.6},{176,-16},{56,-16},{56,-10.44},{5.74,
  ...
  connect(buildingRCZ1Valve.ports_bHeaWat[1], tan.fluPorVol1[10]) annotation (
      Line(points={{326,-67.6},{344,-67.6},{344,6},{56,6},{56,10.8},{5.74,10.8}},
        color={0,127,255}));
  ...
  connect(buildingRCZ1Valve.weaBus, weaDat.weaBus) annotation (Line(
      ...
      color={255,204,51},
      thickness=0.5));
  connect(tan.T1, TTop) annotation (Line(points={{18.72,29.68},{34,29.68},{34,
          222},{354,222},{354,221},{347,221}},
                                  color={0,0,127}));
  connect(tan.T5, TMiddle) annotation (Line(points={{18.13,-2.77},{24,-2.77},{
          24,182},{348,182}},              color={0,0,127}));
  connect(tan.T10, TBottom) annotation (Line(points={{18.13,-45.25},{216,-45.25},
